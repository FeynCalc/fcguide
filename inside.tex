...
This causes the program file "FeynCalc.m" to be loaded. This file contains definitions of the core objects, used by higher level, functions. The more important of these objects are: \mb{DiracGamma}, \mb{FeynAmpDenominator}, \mb{FourVector}, \mb{LorentzIndex}, \mb{Momentum}, \mb{NonCommutative}, \mb{Pair}, \mb{PartialD}, \mb{PropagatorDenominator}, \mb{QuantumField}, S\mb{UNIndex}, \mb{ScalarProduct}, \mb{Spinor}.

"FeynCalc.m" also scans the subdirectories of the directory "HighEnergyPhysics" for files with an extension ".m". Each such file, e.g. "FeynRule.m", should contain the definition of a function, e.g. \mb{FeynRule}. 
...
\item{{\bf \fctables.} Databases of lagrangians, amplitudes and integrals. The more important functions are: \mb{B0}, \mb{C0}, \mb{Lagrangian}, \mb{Amplitude}, \mb{Integrate2}.}

\item{{\bf \fcloops.} Tools for calculating loop integrals. The more important functions are: \mb{ OneLoop}, \mb{PaVeReduce}.}

\item{{\bf \fctools.} Various tools for working with quantum fields and amplitudes. The more important functions are: \mb{Contract}, \mb{DiracReduce}, \mb{FunctionalD}, \mb{FeynRule}, \mb{Tr}, \mb{FermionSpinSum}.}
...
\item{{\bf \qcd.} Tools for working with QCD. The more important functions are: \mb{???}.}

\end{itemize}

The governing idea of the file layout of \fc is modularity. The files belonging to each module are in a separate directory; each file corresponds to one function. If a function uses some support functions or options that are not used by other functions but desirable to have globally accessible, one may put these in the same context as the function and add them to the list \mb{multifunpack} in "FeynCalc.m". The dependence on functions in other contexts should be clearly stated with \mb{MakeContext} statements at the beginning of each file.
...
Others can be set at runtime; e.g. \mb{\$Color, \$Covariant, \$Gauge, \$NonComm, \$Abbreviations}. \mb{\$BreitMaison} and \mb{\$Larin} are special cases in that they are set at runtime, but, once set cannot be changed.
...
\mb{\$Gauge} & \mb{1} (Feynman gauge) &  the gauge fixing parameter of QED in Lorentz gauge. Notice that \mb{\$Gauge} is used by some functions, the option \mb{Gauge} by others \cr
...
\mb{\$Abbreviations} & \mb{\{"\phat" $\rightarrow$ "", "*" $\rightarrow$ "", 
 ..., 
 %"\{" $\rightarrow$ "", "[" $\rightarrow$ "", "\}" $\rightarrow$ "", 
 %"]" $\rightarrow$ "", "/" $\rightarrow$ "", "{\bksl}n" $\rightarrow$ "", "{\bksl}r" $\rightarrow$ "", " " $\rightarrow$ "", 
 %", " $\rightarrow$ "", "AxialVector" $\rightarrow$ "AV", "Fermion" $\rightarrow$ "F", 
 %"Momentum" $\rightarrow$ "", "Pair" $\rightarrow$ "", "ParticleMass" $\rightarrow$ "m", 
 %"PseudoScalar" $\rightarrow$ "PS", "RenormalizationState" $\rightarrow$ "", 
 %"Scalar" $\rightarrow$ "S", "SmallVariable" $\rightarrow$ "sma", "Subscript" $\rightarrow$ "su", 
"Vector" $\rightarrow$ "V"\}} &  a list of string substitution rules used when generating names for storing intermediate results. Used by \mb{OneLoop} and \mb{PaVeReduce} \cr
...
\fc is designed in such a way that certain intermediate results are 
...
Setting \mb{\$BreitMaison} to \mb{True} changes the commutation behavior of 
$\gamma^{5}$\footnote{
...
\mbs{DataType[{\sl x1}, {\sl x2}, ..., {\sl  type}]} & defines the symbols {\sl x1}, {\sl x2}, \ldots to have data-type {\sl  type} \cr}{The function carrying the knowledge of \fc data-types.}

\otabtwo{
\mbs{NonCommutative} & a variable treated as noncommutative by \mb{Dot}, ``.'' \cr
...
Declare some symbols \mb{m} and \mb{a} to be of data-types \mb{even} and \mb{odd}.
}
...
For some purposes, like debugging or hacking the \fc source, however, it may be useful to know how the various objects are represented in \fc.
...
\mbs{Momentum[{\sl p}]} & is a four-dimensional momentum. For other than four dimensions: 
\mb{Momentum[{\sl p}, {\sl d}]}. \mb{Momentum[{\sl p}, 4]} simplifies to \mb{Momentum[{\sl p}]} \cr
\mbs{LorentzIndex[$\mu$]} & is a four-dimensional Lorentz index. When $\mu$ is an integer, it evaluates to mb{ExplicitLorentzIndex[$\mu$]}. For other than four dimensions: 
...
%Clear \mb{\$PrePrint}.
%}
\dtog{{SetOptions[\$FrontEnd, "CommonDefaultFormatTypes"->{"Output" -> StandardForm}]
\\
}}{}{
...
\domtog{{DiracMatrix[$\mu$, Dimension$\rightarrow$D]\\
}}{ DiracGamma[LorentzIndex[$\mu$, D], D]\hfil\\
}{
Here is a $\gamma^{\mu}$ in $D$ dimensions.
}
\domtog{{DiracSlash[q] . DiracSlash[q, Dimension $\rightarrow$ D]\\
...
metric tensors; in this case in  \mb{D} dimensions.
The dimension is provided as an argument.
}
\domtog{{\% /. D $\rightarrow$ 4\\
}}{\ Pair[LorentzIndex[$\mu$], LorentzIndex[$\nu$]]\hfil\\
}{
Changing \mb{D} to 4 recovers the
...
\domtog{{FourVector[p, $\mu$, Dimension $\rightarrow$ D - 4] // FeynCalcInternal\\
...
\beom
\domtog{{FeynCalcExternal[DiracMatrix[$\mu$], FinalSubstitutions $\rightarrow \{\mu \rightarrow mu \}$]\\
}}{ GA[mu]\hfil\\
}{The external output form of $\gamma^\mu$.}
\domtog{{FeynCalcInternal[\%, FinalSubstitutions $\rightarrow \{mu \rightarrow \sigma \}$]\\
}}{ DiracGamma[LorentzIndex[$\sigma$]]\hfil\\
}{Translation back to the internal representation.}
\dtog{{SetOptions[\$FrontEnd, "CommonDefaultFormatTypes"->{"Output" -> TraditionalForm}]
\\
}}{}{
...
More in-depth information about e.g. \mb{FeynRule} can be obtained by typing in FeynRule in the AddOns part of the help browser (see figure \ref{help}).
