...
\mb{$\hbox{\sl m}_{0}^2$,\ $\hbox{\sl m}_{1}^2$,\ ...}, determines whether a one-, two-,
...
\beom
\dtog{SetOptions[A0,\ A0ToB0\ $\rightarrow$\ False];\\
\ SetOptions[\{B1,\ B00,\ B11\},\ BReduce\ $\rightarrow$\ False];
}{}{
...
This is $B_{11}(p_1^2,m_0,m_1)$, the coefficient 
...
if the option \mb{BReduce} is specified. Setting the option 
...
Arguments of two-point functions with head \mb{Small} are replaced 
by 0, if the other arguments have no head \mb{Small} 
...
\domtog{{c12\ =\ PaVeReduce[\ PaVe[1,\ 2,\ \{s,\ m2,\ m2\},\ \{m2,\ m2,\ M2\}],\\
IsolateNames\ $\rightarrow$\ c\ ]\ \\
}}{\ $c(11)$\hfil\\
}{
...
\domtog{{c[11]\\
}}{\renewcommand{\clineskip}{36pt}\Frac{2\ {\rm m}2\ c(3)}{s\ c(6)}\ + \ \Frac{c(1)\ c(7)}{s\ \Superscript{c(6)}{2}}\ + \ \Frac{c(2)\ c(8)}{2\ \Superscript{c(6)}{2}}\ + \ \Frac{c(4)\ c(9)}{s\ c(6)}\ + \ \Frac{{\rm M}2\ c(5)\ c(10)}{\Superscript{c(6)}{2}}\ + \ \Frac{1}{2\ c(6)}\hfil\\
\renewcommand{\clineskip}{20pt}}{
...
\dtog{SetOptions[B1,\ BReduce\ $\rightarrow$\ True];}{}{Have $B_1$'s be reduced to $B_0$'s.}
\domtog{{Simplify\ /@\ Collect[FixedPoint[ReleaseHold,\ c12],\ {\_B0,\ \_C0}]\\
}}{\renewcommand{\clineskip}{36pt}\Frac{1}{2\ (4\ {\rm m}2\ -\ s)}\ +\ \Frac{({\rm m}2\ -\ {\rm {\rm M}}2)\ B_0(0,\ {\rm m}2,\ {\rm M}2)}{2\ {\rm m}2\ (4\ {\rm m}2\ -\ s)}\ -\ \\
\Frac{(8\ \Superscript{{\rm m}2}{2}\ \ -\ 10\ {\rm m}2\ {\rm M}2\ -\ 2\ {\rm m}2\ s\ +\ {\rm M}2\ s)\ B_0({\rm m}2,\ {\rm m}2,\ {\rm M}2)}{2\ {\rm m}2\ \Superscript{(4\ {\rm m}2\ -\ s)}{2}}\ \\
+\ \Frac{(4\ {\rm m}2\ -\ 6\ {\rm M}2\ -\ s)\ B_0(s,\ {\rm m}2,\ {\rm m}2)}{2\ \Superscript{(4\ {\rm m}2\ -\ s)}{2}}\ \\
+\ \Frac{{\rm M}2\ (8\ {\rm m}2\ -\ 3\ {\rm M}2\ -\ 2\ s)\ C_0({\rm m}2,\ {\rm m}2,\ s,\ {\rm m}2,\ {\rm M}2,\ {\rm m}2)}{\Superscript{(4\ {\rm m}2\ -\ s)}{2}}\hfil\\
\renewcommand{\clineskip}{20pt}}{
...
\ Mandelstam\ $\rightarrow$\ \{s,\ t,\ u,\ 2\ mw2\},\ IsolateNames\ $\rightarrow$\ f\ ]\ \\
}}{\ f[16]\hfil\\
}{
...
The scaling variable $\mu$ is suppressed in 
\fc.
}
...
\dtog{{SetOptions[A0,\ A0ToB0\ $\rightarrow$\ True];\\
}}{}{
...
\dtog{{SetOptions[OneLoop,\ Factoring\ $\rightarrow$\ True];\\
}}{}{
For a most compact result the factoring option of \mb{OneLoop} is set.
...
]\ /.\ ScalarProduct[k,\ k]\ $\rightarrow$\ k2\ /.\\
(mf\phat 2)\ $\rightarrow$\ mf2\\
}}{
\Frac{
...
DiracMatrix[nu]\ .\ gc[2]\ .\\
Spinor[p2]\ \ ]\ /.\ (m\phat 2)\ $\rightarrow$\ m2\\
}{
...
\mb{StandardMatrixElement}, which displays like
...
g[i\_,\ "+"]\ $\rightarrow$\ gp[i]\ /.\\
g[i\_,\ "-"]\ $\rightarrow$\ gm[i],\ FormatType\ $\rightarrow$\ FortranForm];\\
}}{}{
...
\otabtwo{
\mbs{StandardMatrixElement[ {\sl expr}]} & head of a standard matrix element \cr
\hspace*{-18pt}\mbs{SetStandardMatrixElements[\{\ \{{\sl sm1}\ $\rightarrow$\ {\sl abb1}\},\ \{{\sl sm2}\ $\rightarrow$ \ {\sl abb2}\},\ ...\}]}&
set abbreviations for standard matrix elements \cr
\hspace*{-18pt}\mbs{SetStandardMatrixElements[\{\ \{{\sl sm1}\ $\rightarrow$\ {\sl abb1}\},\ \{{\sl sm2}\ $\rightarrow$ \ {\sl abb2}\},\ ...\}}, 
$k_2 \rightarrow p_1+p_2-k_1$\mb{]}& set abbreviations  for standard matrix elements by using 
energy momentum conservation 
} {A head for identifying standard matrix elements; $sm1, sm2$ are the standard
matrixelements, $abb1, abb2$ the abbreviations.}
...
ScalarProduct[p1,\ p2]\ =\ k2/2\ ;\\
SetOptions[\ OneLoop,\ \\
Factoring\ $\rightarrow$\ True,\ FormatType\ $\rightarrow$\ FortranForm,\\
ReduceToScalars\ $\rightarrow$\ True,\ WriteOut $\rightarrow$ True,\\
FinalSubstitutions\ $\rightarrow$\ \{g[i\_,\ "+"]\ $\rightarrow$\ gp[i],\ 
g[i\_,\ "-"]\ $\rightarrow$\ gm[i],\\
StandardMatrixElement\ $\rightarrow$ \ ma\}\ ];\\
mt\ =\ MetricTensor;\ fv\ =\ FourVector;\\
feynden[x:\{\_,\ \_\}..]\ :=\ FeynAmpDenominator\ @@\ \\
Map[Apply[PropagatorDenominator,\ \#]\&,\ \{x\}];\\
}}{}{
In addition to the first example the option 
\mb{ReduceToScalars} is set to \mb{True},   
which will produce directly a result  in 
...
r\ .\ Spinor[p2]\ )\ $\rightarrow$\ \{1\},\\
(\ \ Spinor[p1]\ .\ DiracSlash[Polarization[k]].\ 
l\ .\ Spinor[p2]\ )\ $\rightarrow$\ \{2\}\}];\\
}}{}{
...
]\ /.\ (m1\phat 2)\ $\rightarrow$\ m12\ /.\ (m2\phat 2)\ $\rightarrow$\ m22\\
}{
...
is replaced by \mb{mat}, as specified in the option
\mb{FinalSubstitutions} of \mb{OneLoop} on the previous page.
}
\domtog{{!!"wff2.for"\\
}}{\scriptsize\tt
...
a Fortran output is written into a file.
}
...
in Section \ref{self}, you have to set \mb{Dimension\ $\rightarrow$\ D}.
...
In case you want to calculate a finite amplitude, you can set \mb{Dimension\ $\rightarrow$\ 4}.
...
\mb{InitialSubstitutions\ $\rightarrow$\ \{k2\ $\rightarrow$\ -\ k1\ \ +\ p1\ +\ p3\}}.
...
\mb{Mandelstam\ $\rightarrow$\ \{{\sl s},\ {\sl t},\ {\sl u},\ {\sl m1}\phat 2\ +\ {\sl m2}\phat 2\ +\ {\sl m3}\phat 2\ +\ {\sl m4}\phat 2\}}, where  $s + t + u = m_1^2 + m_2^2 + m_3^2 + m_4^2$.
...
If \mb{WriteOut} is set to \mb{True}, the result is written out into a file composed of the first
argument of \mb{OneLoop}, i.e., the $name$. In which language, i.e., 
\mma, \mb{Fortran}, \mb{Macsyma} or \mb{Maple} the result is written, depends on the setting of the option \mb{FormatType}.
...
\mb{InitialSubstitutions} & \mb{\{\}} & substitutions done at the beginning of the calculation, i.e., energy momentum conservation \cr
...
\mb{Sum} &  \mb{True} & compatibility with \fa; sum terms multiplied with \fa's \mb{SumOver} \cr
...
subsequently summed, or the summation occurs partially before calculation.
This can be specified with the option \mb{CombineGraphs}.

\otabtwo{
\mbs{OneLoopSum[List[FeynAmp[ GraphName[...,\ {\sl N1}],\ {\sl q},\ {\sl amp1}], 
...
The input of \mb{OneLoopSum} is adapted to the output of \fa \footnote{Actually to version 1 of \fa, but the current version 3 provides translation to version 1 syntax (\mb{ToFA1Conventions}). The examples given here all use \fa version 3,} .
After saving a list of Feynman diagrams (that is, unintegrated amplitudes)
created with \fa\ using the function 
\mb{CreateFeynAmp}, i.e. \mb{CreateFeynAmp[{\sl ins}] // PickLevel[Particles] // ToFA1Conventions >> {\sl filename}},
you can start a new \mma session 
\footnote{It is not possible to load \fc and \fa\ 
simultaneously into one \mma session because some functions of \fa and \fc have the same name but are in different contexts (name spaces). A more sophisticated approach is provided by the optional subpackage PHI, which patches \fa slightly, allowing the two packages to be loaded simultaneously.},
load \fc, get the amplitudes by \mb{{\sl amps}\ =\ << {\sl filename}} 
...
, calculate the amplitude with \mb{OneLoopSum} and finally save the result e.g. as a Fortran file.
...
give the  sum of already calculated ones as argument ($expr$) to \mb{OneLoopSum}.
...
\otabtwo{
\mbs{FRH[{\sl exp}]} & removes all \mb{HoldForm} and \mb{Hold} in {\sl exp}
}{A utility function.}
...
You may set, e.g., \mb{SelectGraphs\ $\rightarrow$\ \{1,\ 2,\ 5,\ 8\}}, which selects the 
amplitudes at positions 1, 2, 5 and 8 of the argument list of \mb{OneLoopSum}.
The setting \mb{SelectGraphs\ $\rightarrow$\ \{1,\ 2,\ 5,\ 8,\ \{10,\ 40\}\}} also includes the range of all amplitudes from position 10 to 40.
...
the result of the reduction of each \mb{PaVe} in a file for later use.
...
\mb{OneLoopSum} has then as a global factor $1/(2 s_w^2)$.
...
\end{scriptsize}
\begin{tabular*}{0.8\textwidth}{c}\hhline\end{tabular*}
\GrayCaption{Mathematica output file "eezhb.s" generated by \mb{OneLoopSum} for ${\rm e}^+{\rm e}^- \rightarrow  {\rm ZH}$.}
\end{figure}
