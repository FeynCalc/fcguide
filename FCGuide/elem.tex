...
You can use \fc for basic calculations like Lorentz algebra and Dirac and color trace
evaluations. This section contains simple examples of such calculations.
...
}{Here is another product. Notice that we are using the shorthand \mb{FeynCalcExternal} form (see section \ref{inside}) for input.}
...
As mentioned in section \ref{gamma5}, basic features of the Breitenlohner-Maison scheme \cite{Breitenlohner:hr} (for a short explanation of the Breitenlohner-Maison symbols like $\hat{\gamma}^\mu$, see e.g.~\cite{Martin:1999cc}) are implemented. Below are given some simple illustrations.
...
\mb{FeynCalcExternal}         & \mb{0} & give output in \mb{FeynCalcExternal} form (see section \ref{inside}) \cr
\mb{EpsContract} & \mb{False} & contract Levi-Civita \mb{Eps} \cr
