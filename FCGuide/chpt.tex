...
\otabtwo{
\mbs{FieldDerivative[g[x], x, \{{\sl l1}, {\sl l2}, ...\}]} & calculates
$\frac{\partial}{\partial x_{l_1}}\frac{\partial}{\partial {x}_{l_2}}\ldots$ on
the expression \mb{g[{\sl x}]}\cr
\mbs{CovariantFieldField Derivative[g[{\sl x}], {\sl x}, \{{\sl l1}, {\sl l2}, ...\}]} &
...
\mb{QuantumField}s may be grouped in flavor SU($N$) matrices or vectors\footnote{with SU($N$) we shall understand  SU($N$), where $N$=2 or 3.} with the functions \mb{IsoVector}, \mb{UMatrix} and \mb{UVector}. When this is done, these functions take over the space-time argument.
...
\hhline
\mbs{SUNN} & \mb{2} & number of quark flavors. Either 2 or 3 \cr
...
The matrices generating flavor SU($N$) are denoted \mb{UGenerator}.
...
To calculate the trace of flavor SU($N$) matrices, the function \mb{Utrace} is used.
...
\otabtwo{
\mbs{UTrace[{\sl m}]} & calculate the flavor SU($N$) trace of the matrix {\sl m} \cr
...
\otabtwo{
\mbs{DeclareUMatrix[{\sl a}]} & causes the symbol {\sl a} to be treated as a flavor SU($N$) matrix \cr
\mbs{DeclareUScalar[{\sl a}]} & causes the symbol {\sl a} to be treated as a flavor SU($N$) scalar \cr
\mbs{UndeclareUMatrix[{\sl a}]} & causes the symbol {\sl a} to seize being treated as a flavor SU($N$) matrix \cr
\mbs{UndeclareUScalar[{\sl a}]} & causes the symbol {\sl a} to seize being treated as a flavor SU($N$) scalar \cr
...
function, \mb{XName}, is provided for generating the correct name.
Storing and retrieving from the directory "HighEnergyPhysics/Phi/CouplingVectors" can then
be done with the function \mb{CheckDB}.
...
\mbs{CheckDB[{\sl exp}, {\sl fil}]} & checks if {\sl fil} exists.  If it does, gets {\sl fil} and
